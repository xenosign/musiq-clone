...
export default function RoomPage() {
  ...
  const handleOpen = useCallback(() => {
    ...
    sendRef.current({
      type: 'join_room',
      payload: { roomId, playerName, password: roomPasswordParam },
    });
  }, [roomId, playerName, roomPasswordParam, router]);
  ...
  const submitReport = async (message: string) => {
    send({ type: 'report_music_error', payload: { message } });
      ...
      <header className="bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <button
              onClick={() => setShowReportPopup((v) => !v)}
              className="text-sm bg-gray-700 hover:bg-gray-600 transition px-3 py-1 rounded-lg font-semibold"
            >
              🚨 음악 오류 제보
            </button>
            {showReportPopup && (
              <div className="absolute right-0 top-full mt-2 w-72 bg-gray-800 border border-gray-700 rounded-xl shadow-xl p-4 flex flex-col gap-3 z-50">
                <p className="text-sm text-gray-300 leading-relaxed">
                  오류가 있는 음악 제목만 보내주셔도 충분히 체크가 가능합니다!!
                  음악 오류 제보는 해당 서비스 개발에 큰 힘이 됩니다.
                  감사합니다! 🙇‍♂️
                </p>
                <input
                  type="text"
                  ...
                  onKeyDown={(e) => {
                    if (
                      ...
                      submitReport(reportInput.trim());
                      setReportInput('');
                      setShowReportPopup(false);
                    }
                  }}
                  placeholder="오류 내용을 입력해주세요"
                  className="w-full bg-gray-700 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#FF9900]"
                  autoFocus
                />
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 bg-gray-700 hover:bg-gray-600 transition px-3 py-1.5 rounded-lg text-sm"
                  >
                    취소
                  </button>
                  <button
                    ...
                    className="flex-1 bg-orange-600 hover:bg-orange-700 transition px-3 py-1.5 rounded-lg text-sm font-semibold"
                  >
                    전송
                  </button>
                </div>
          ...
          <button
            onClick={handleLeave}
            className="text-sm bg-orange-600 hover:bg-orange-700 transition px-3 py-1 rounded-lg font-semibold"
          >
            나가기
          </button>
        </div>
              ...
              <div className="p-4 flex flex-col gap-3">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 relative">
                    {/* 정답 패널 — 항상 렌더링하여 컨테이너 높이 고정 */}
                    <div
                      className={`bg-orange-600/10 border border-[#FF9900] rounded-xl p-4 flex flex-col items-center gap-1 ${!lastCorrect ? 'invisible' : ''}`}
                    >
                    ...
                    {/* 힌트 패널 — 정답 전까지 absolute로 오버레이 */}
                    <div
                      className={`absolute inset-0 bg-gray-800 rounded-xl p-4 flex flex-col items-center justify-center gap-2 ${lastCorrect ? 'invisible' : ''}`}
                    >
